...
var numallatage = document.getElementsByTagName("a").length;
for (let m = 0; m < numallatage; m++) {
    var ahref = document.getElementsByTagName("a")[m].href;
    var doorno = false;
    if (scriptmode == "Exclude") {
        doorno = true;
    } else {
        doorno = false;
    }

    for (let h = 0; h < listofsites.length; h++) {
        if (ahref.indexOf(listofsites[h]) > -1) {
            if (scriptmode == "Exclude") {
                doorno = false;
            } else {
                doorno = true;
    ...
    if (doorno == true) {
        var cahref = aesCrypto.encrypt(ahref, "5aithamtech.blogspot.com");
        document.getElementsByTagName("a")[m].href = "https://" + gourl + "/search?q=1#" + cahref;
    }
}
